<?php
    ...
    if(isset($_SESSION['datesearch'])){
        ...
        $getsalecount =mysqli_query($dbconnect,"SELECT * FROM `tbl_sales` WHERE `sale_soldby`='$sidno' AND `datee`>='$startdate' AND `datee`<='$enddate' group by `sale_itemcode`");
        ...
        $total= $row["total"];
        $sql5 = "SELECT SUM(sale_amount) as mpesa FROM tbl_sales WHERE `sale_soldby`='$sidno' && sale_modeofpayment='Mpesa' AND `datee`>='$startdate' AND `datee`<='$enddate'";
        $result5 = mysqli_query($dbconnect, $sql5);
        $row5 = mysqli_fetch_assoc($result5);
        $mpesa= $row5["mpesa"];
        $sql7 = "SELECT SUM(sale_amount) as cash FROM tbl_sales WHERE `sale_soldby`='$sidno' && sale_modeofpayment='Cash' AND `datee`>='$startdate' AND `datee`<='$enddate'";
        $result7 = mysqli_query($dbconnect, $sql7);
        $row7 = mysqli_fetch_assoc($result7);
        $cash= $row7["cash"];
        $other=$total-($mpesa+$cash);?>
        <div class="ibox-contenti p-xl printed">
                                    <table width="100%" cellspacing="0" cellpadding="4" style="font-size:20px;border-radius:5px;">
                                        <tr>
                                        ...
                                        <tr style="color: green; border-top:2px solid black; margin-top: 2px;">
                                            <th>Total - </th>
                                            <th><?php if($total>0){echo $total."/=";}else{
                                                echo "0.00";
                                            }?></th>
                                        </tr>
                                    </table>
