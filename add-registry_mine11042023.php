											...
											<span></span>
													...
													if(isset($_POST['newpatient']) || isset($_POST['newpatient1'])){
														if(empty($_POST['firstname'])){
															...
															if($countNo >= 1){
																...
																if($stmt = $dbconnect->prepare("INSERT INTO tbl_registry (id_no, f_name, l_name, phone_no, gender, dob, residence, opno, visit_date,visit_no,scheme_code,visit_status,agecategory,age,parentFname,parentLame,memberno) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")){
																	$stmt->bind_param('sssssssssssssssss', $cidno, $cfname, $clname, $cphone, $cgender, $cdob, $creside, $opno, $cvisit,$newvisit,$scheme,$visit_status,$agecat,$agee,$cparentfname,$cparentlname,$memberno);
																	$stmt->execute();
												...
												?>
													...
													<div class="col-sm-6" id="adultForm" style="display:block;">
														<div class="form-group">
															<label>Phone Number</label>
															<input type="text" name="phonenumber1" placeholder="You Phone Number" class="form-control">
														</div>
														<div class="form-group">
															<label>ID No</label>
