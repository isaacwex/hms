   ...
   <title>Sales Patients - <?php echo $smart_name; ?></title>
	...
	<script>
		function showResult(str) {
			...
			xmlhttp.open("GET","adminsalesprocessor.php?page=searchsales&q="+str,true);
			xmlhttp.send();
			}
		...
		function showResult1(str) {
			...
			xmlhttp.open("GET","adminsalesprocessor.php?page=searchdsales&q="+str,true);
			xmlhttp.send();
			}
			var xmlhttp=new XMLHttpRequest();
			xmlhttp.onreadystatechange=function() {
		...
		function showResult2(str) {
			...
			xmlhttp.open("GET","adminsalesprocessor.php?page=searchUser&q="+str,true);
			xmlhttp.send();
			}
			var xmlhttp=new XMLHttpRequest();
			xmlhttp.onreadystatechange=function() {
        ...
        <script>
			...
			xmlhttp.open("GET","adminsalesprocessor.php?page=receiptdetails&i="+b);
			xmlhttp.send();
			}
		</script> 
		...
		<script>
			...
			xmlhttp.open("GET","adminsalesprocessor.php??page=searchdsales&i="+b);
			xmlhttp.send();
			}
		</script>
		<script>
		...
		<script>
			...
			xmlhttp.open("GET","adminsalesprocessor.php?str="+str);
			xmlhttp.send();
			}
		</script>
		<script>
		...
		<script>
			...
			xmlhttp.open("GET","salestotal.php?page=totalsales");
			xmlhttp.send();
			}
		</script>
				...
				<div class="col-lg-1"> 
					...
					<div class="row">					
						...
						<label><span class="badge badge-success">Search Receipt numbers within the selected dates </span></label>
							<input type="text" size="30" onkeyup="showResult(this.value)" placeholder="Search Invoice Number" class="form-control"></div>	
						</div>
						<div class="col-lg-4"><div class="form-group">
						...
						<div class="col-lg-4"><div class="form-group">
						<label><span class="badge badge-success">search user sales </span></label>
                            <select data-placeholder="Choose drug..." name="drug" onchange="showResult2(this.value)" class="form-control chosen-select">
														
														<option selected value="">Select Users from List </option>
														<option value="*">All Users</option>
														<?php
													$getalllocations = mysqli_query($dbconnect,"SELECT * FROM tbl_users");
													while($gal = mysqli_fetch_array($getalllocations)){
														$product_code = $gal['id_no'];
														$brand_name = $gal['s_name'];
														?>
														<option value="<?php echo $product_code; ?>" ><?php echo $brand_name; ?></option>
														<?php
													}
                        ...
                        <div class="ibox-content" style="overflow:auto;height:78vh; -ms-overflow-style: none; scrollbar-width: none;">
                           ...
                           <div id="livesearch" >
							<?php 
								...
								$getcountyname =mysqli_query($dbconnect,"SELECT * FROM `tbl_sales` WHERE `datee`>='$startdate' AND `datee`<='$enddate' group by `sale_receiptno`");if ($getcountyname->num_rows > 0) {?>
