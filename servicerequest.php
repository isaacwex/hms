...
<head>
    ...
    <title>Service Requests - <?php echo $smart_name; ?></title>
             ...
             <div class="col-lg-12">
                                <div class="modal-content">
                                        <div class="modal-header">											
                                            ...
                                            <h2 class="modal-title"> <b>Requests</b></h2>
                                            <small class="font-bold">Create Patient Requests - <?php echo $patientcategory; ?></small>
                                        </div>
                                        <div class="modal-body">
											...
											<form role="form" method="post">	
												...
												<?php
													...
													$agess = $diff->format('%y');
													if(isset($_POST['btn_servicerequest'])){
														$sq_servicecode = $dbconnect->real_escape_string($_POST['servicecode']);
														$requestnote = $dbconnect->real_escape_string($_POST['requestnote']);
														$sq_requesttime=date('Y/m/d');
														$sq_requester=$sidno;
														$sq_requestfrom=$current_processstage;
														$request_status = 'OPEN';
														$opno1=$opno;
														$visitno1=$visitno;
														
											//Getting Service Details for Insertion	
														$servicedet =mysqli_query($dbconnect,"SELECT * FROM tbl_services s INNER JOIN tbl_service_prices p ON s.ss_code=p.sp_code WHERE p.sp_schemecode='$scheme_code' AND s.ss_code='$sq_servicecode' LIMIT 1");
											...
											//start of the actual insertion into sevices and bill creation			
														$checkL = mysqli_query($dbconnect, "SELECT * FROM tbl_servicerequests WHERE sq_opno='$opno' AND sq_visitno='$visitno' AND sq_servicecode='$sq_servicecode' AND sq_status='OPEN'");
														...
														if($countL>=1){
															...
															if($stmt = $dbconnect->prepare("INSERT INTO tbl_servicerequests (sq_opno, sq_visitno,sq_servicecode,	sq_requestnote,sq_status,sq_servicecategory,sq_requesttime,sq_requester,sq_requestfrom,sq_price,sq_patientcategory) VALUES (?,?,?,?,?,?,?,?,?,?,?)")){
																	$stmt->bind_param('sssssssssss',$opno1, $visitno1,$sq_servicecode,$requestnote,$request_status,$sq_servicecategory,$sq_requesttime,$sq_requester,$sq_requestfrom,$service_price,$patientcategory);
																	$stmt->execute();
													//Updating the bill				
																																	
																		$billstatus = 'INITIATED';
																		...
																		VALUES ('$service_name', '$service_price','$opno1', '$visitno1','$scheme_code','$billstatus','$sq_servicecategory','$service_code','$patientcategory')";
												...
												<div class="col-md-6">	
														<div class="col-md-12"> 
															...
															<select name="servicecode" required class="form-control chosen-select" >
																	...
																	<?php
																$getalllocations = mysqli_query($dbconnect,"SELECT * FROM tbl_services s INNER JOIN tbl_service_prices p ON s.ss_code=p.sp_code WHERE p.sp_schemecode='$scheme_code' AND s.ss_category!='LABORATORY' AND s.ss_category!='TREATMENTROOM' AND p.sp_price>0");
														...
														<div class="col-md-12">
															<div class="form-group">
																...
																<textarea name="requestnote" class="form-control" placeholder="Enter request note (optional)" rows="1"></textarea>
															</div>
														</div>									
														<div class="col-md-6">
														</div>
														<div class="col-md-6">
															<div class="form-group pull-right">
																<input name="btn_servicerequest" class="btn btn-md btn-success" type="submit" value="Create Request"/>
															</div>
														</div>
													</div>
													</br>
													...
													<?php
														...
														<?php
															if(isset($_GET['delete'])){
																$deleted = $_GET['delete'];
																$action = mysqli_query($dbconnect,"DELETE FROM tbl_servicerequests WHERE sq_id='$deleted'");
																if($action){
																	...
																	<script>
																		...
																		window.location ='servicerequest.php?opno=<?php echo $opno; ?>&visitno=<?php echo $visitno; ?>&current_processstage=<?php echo $current_processstage;?>&c_id=<?php echo $c_id;?>';
																	</script>	
																	<?php
																}
																else {
														...
														if($sq_status=='OPEN'){	
															?>
														<a href="servicerequest.php?opno=<?php echo $opno; ?>&visitno=<?php echo $visitno; ?>&current_processstage=<?php echo $current_processstage;?>&c_id=<?php echo $c_id;?>&delete=<?php echo $labrequest_id; ?>"><button type="button" class="btn-xs btn-danger" onclick="return confirm('Are you sure you want to delete the item on the list?')" > <span class="fa fa-trash"></span></button></a>
														</br>
														<?php
															}
														}
