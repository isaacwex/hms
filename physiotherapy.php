...
<head>
   ...
   <title>Physiotherapy Services - <?php echo $smart_name; ?></title>
...
<body>
	...
	<?php include('includes/sidebar.php');?>
		...
		<?php
			$current_processstage='PHYSIOTHERAPY';
                ...
                <div class="ibox float-e-margins">
					...
					<table class="table table-striped table-bordered table-hover dataTables-example">
						...
						<tr>						
							<th>Names</th>
							<th>Details</th>
							<th>Pending Physiotherapy Requests  </th>
							<th>Actions </th>
						</tr>
						</thead>
						<tbody>
						...
						<?php "<tr class='gradeX'>"; ?>
							...
							<td>
							<?php
								$getvitals = mysqli_query($dbconnect, "SELECT * FROM tbl_servicerequests q INNER JOIN tbl_services v ON q.sq_servicecode=v.ss_code WHERE q.sq_opno='$opno' AND q.sq_visitno='$visitno' AND q.sq_servicecategory='PHYSIOTHERAPY'");
								...
								$update_item = "UPDATE tbl_servicerequests SET sq_status=? WHERE sq_id='$sq_id'";
										if($stmt = $dbconnect->prepare($update_item)) {
											...
											?>
												<script>
												 alert('Successful');
												window.location = 'Physiotherapy.php';
												</script>	
											<?php
									...
									if($sq_status=='OPEN'){
								?>
											<a href="Physiotherapy.php?opno=<?php echo $opno; ?>&visitno=<?php echo $visitno; ?>&current_processstage=<?php echo $current_processstage;?>&c_id=<?php echo $c_id;?>&closeservice=<?php echo $sq_id;?>" data-toggle="modal" title=" Request results"><span class="badge badge-warning">Close Now <i class="fa fa-arrow-right"></i></span></a>
											</br>
											<?php
								}else{
										?>							
